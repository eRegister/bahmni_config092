...
JOIN obs tr 
    ...
    AND cast(tr.obs_datetime as date) BETWEEN cast(@start_date as Date) AND cast(@end_date as Date)
...
JOIN (
    ...
    WHERE concept_id = 4798
      AND cast(obs_datetime as date) BETWEEN cast(@start_date as Date) AND cast(@end_date as Date)
...
JOIN reporting_age_group ag 
  ON cast(@end_date as Date) BETWEEN DATE_ADD(DATE_ADD(p.birthdate, INTERVAL ag.min_years YEAR), INTERVAL ag.min_days DAY)
